...
3. **Background removal**: auto-detected — if >5% of pixels are already transparent (pre-cut PNG), BG removal is disabled automatically. Toggle the checkbox and adjust tolerance as needed.
4. **Scroll-wheel zoom** to get closer for precise polygon drawing. Zoom % shown in toolbar; click **fit** to reset.
5. **Draw polygon regions** for each bone — click "draw" next to a bone in the left panel, then click on the image to place vertices. Click near the first point (white dot) to close the polygon.
   - Right-click or Esc to undo last vertex
   - Cmd/Ctrl+Z to undo
6. Click **Apply & Build Parts** — the image is clipped per polygon, trimmed, and loaded into the rig editor with smart defaults:
   - **Pivots** (Three.js coords — Y: 0=bottom, 1=top): head → bottom-center (neck), arms → upper-inner (shoulder), legs → upper-inner (hip), torso → upper-center
...
- **Toolbar**: character name input, new button (reset all), PSX shader toggle, export button. The character name is used for the zip filename and manifest name.
...
Click **Export** → downloads a `.zip` (named after the character name field, or `character.zip` by default) containing:
