...
function checkRateLimit(sessionKey: string): { allowed: boolean; retryAfterMs?: number } {
  ...
  for (const [window, limit] of [
    [globalRate.minute, LIMITS.global.minute, 60_000] as const,
    [globalRate.hour, LIMITS.global.hour, 3600_000] as const,
    [globalRate.day, LIMITS.global.day, 86400_000] as const,
  ]) {
    const [w, lim, duration] = [window, limit, duration] as unknown as [RateWindow, number, number]
    if (now > w.resetAt) {
      w.count = 0
      w.resetAt = now + (duration as number)
    }
    if (w.count >= lim) {
  ...
  for (const [key, limit] of [
    ['minute', LIMITS.session.minute] as const,
    ['hour', LIMITS.session.hour] as const,
    ['day', LIMITS.session.day] as const,
  ]) {
    const w = session[key]
