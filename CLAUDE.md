...
- `client-3d/` **(new — Feb 2026)**
  ...
  - Key dirs:
    - `src/scene/` — Room (walls + DJ booth + occlusion), Camera (orbit + sway, exports `cameraDistance` for fisheye scaling), PlayerEntity (lerp + 3D chat bubbles + troika Text nametags on layer 1), GameScene (Canvas + ClickPlane + debug keyboard shortcuts: `` ` `` for FPS, `-`/`=` for render scale cycle)
