...
- `client-3d/` **(new — Feb 2026)**
  ...
  - Key dirs:
    ...
    - `src/stores/` — gameStore (+ `connectionStatus`: `'disconnected'` | `'connected'` | `'reconnecting'`, `selectedCharacterPath`), chatStore (+ bubbles), musicStore, uiStore (+ debug: `showFps`, `renderScale` [0.75/0.5/0.35], `fisheyeOverride`, `vertexFisheye`, `vortexOob`; + `computerIframeOpen`), boothStore (DJ booth + queue + video bg, `videoBackgroundEnabled` defaults `true`)
