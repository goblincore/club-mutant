...
impl<'opts> Video<'opts> {
    ...
    pub async fn get_basic_info(&self) -> Result<VideoInfo, VideoError> {
        ...
        // Use configured client type or default to android_sdkless
        // Options: "web", "android_sdkless", "ios", "tv_embedded"
        if !is_live(&player_response) {
            let client_type = self.options.request_options.client_type
                .as_deref()
                .unwrap_or("android_sdkless");
            let client_ytconfig = self
                .get_player_ytconfig(
                    &response,
                    INNERTUBE_CLIENT
                        .get(client_type)
            ...
            let player_response_new =
                serde_json::from_str::<PlayerResponse>(&client_ytconfig).unwrap_or_default();
