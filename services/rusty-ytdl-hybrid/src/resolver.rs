...
// Note: ytdlp-ejs is available but not needed - rusty_ytdl handles n-param internally
...
pub async fn resolve_video(
    ...
    // Get the URL - rusty_ytdl already handles both signature decryption AND n-parameter
    // transformation internally via its ncode function, so we don't need ytdlp-ejs
